...
8. **관련 문서 최신화**: 테이블 구조 신설/변경, 라우트 신설, 신규 컴포넌체 추가, 디렉토리 구조 변경 등 작업이 성공적으로 완료되면, 실제 코드를 바탕으로 연관된 프로젝트 사양 문서(예: `database_specification.md`, `project_structure.md` 등)를 누락 없이 최신 정보로 갱신하여 문서 일치성을 유지하십시오.
9. **통합 금융 차트 및 시세 API 어댑터 규칙**:
