...
실거래 AWS는 학습을 수행하지 않고, 로컬 머신이 생성·검증한 릴리스만 읽습니다. 코인 신규 매수는 릴리스 생성 시각이 12시간 이내일 때만, 주식 신규 매수는 36시간 이내일 때만 허용합니다. 예측이 만료되면 신규 매수만 보류하며, 보유 포지션의 손절·익절·비상정지·체결 대사는 계속 동작합니다.
