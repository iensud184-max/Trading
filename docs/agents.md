...
### Phase 3: Toss 실거래 반자동 매매

* **구현 범위**: 사용자가 승인 버튼을 누르면 Toss 주문 API를 호출하며 멱등성과 사전검증을 적용합니다.
* **코드 필수 안전 조항**:
...
13. **모의투자 및 실거래 환경 분기**:
   * 모의투자 테스트 및 시뮬레이션(`MOCK` 환경)과 실거래(`REAL` 환경) 환경 간 API 연동 및 사전 검증 결과를 명확히 분기하여 관리해야 합니다.
